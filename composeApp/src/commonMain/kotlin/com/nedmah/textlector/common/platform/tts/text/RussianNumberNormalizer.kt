...
object RussianNumberNormalizer {
    ...
    private val yearPrepositional = Regex(
        """\b(в|на)\s+(\d{4})\s+(году)\b""",
        RegexOption.IGNORE_CASE
    )
    private val yearGenitive = Regex(
        """\b(с|до|после)\s+(\d{4})\s+(года)\b""",
        RegexOption.IGNORE_CASE
    )
    private val masculineOrdinalPrepositional = Regex(
        """\b(в|на)\s+(\d{1,4})\s+(этаже|месте|уровне|пункте|разделе)\b""",
        RegexOption.IGNORE_CASE
    )
    private val feminineOrdinalPrepositional = Regex(
        """\b(в|на)\s+(\d{1,4})\s+(странице|главе|строке|части)\b""",
        RegexOption.IGNORE_CASE
    )

    private val governedNumber = Regex(
        """\b(около|возле|вокруг|без|до|из|от|у|для|после|кроме|к|ко|с|со|над|под|перед|между|о|об|обо|при)\s+(\d{1,6})(?:\s*(км|кг|см|мм|м|руб\.?|рубль|рубля|рублей|₽|%))?""",
        RegexOption.IGNORE_CASE
    )
