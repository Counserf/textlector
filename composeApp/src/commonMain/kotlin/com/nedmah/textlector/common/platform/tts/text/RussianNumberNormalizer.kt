...
object RussianNumberNormalizer {
    ...
    private val governedNumber = Regex(
        """(?<![А-Яа-яЁё0-9_])(около|возле|вокруг|без|до|из|от|у|для|после|кроме|к|ко|с|со|над|под|перед|между|о|об|обо|при)\s+(\d{1,6})(?:\s*(км|кг|см|мм|м|руб\.?|рубль|рубля|рублей|₽|%))?""",
        RegexOption.IGNORE_CASE
    )
